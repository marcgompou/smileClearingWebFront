...
export class PoidsValidationWorkflowService {
  private _poidsValidationWorkflow: BehaviorSubject<
    PoidsValidationWorkflow[] | null
  > = new BehaviorSubject(null);
  private _client: BehaviorSubject<PoidsValidationWorkflow | null> =
    new BehaviorSubject(null);
  private _poidsValidationUser: BehaviorSubject<number[] | null> =
    new BehaviorSubject(null);
  /**
   * Constructor
   */
  ...
  /**
     ...
     * @returns 
     */
  createPoidsValidationWorkflow(
    poidsValidationWorkflow: PoidsValidationWorkflow
  ): Observable<any> {
  ...
  get(): Observable<any> {
    return this._httpClient
      .get<any>(`${environment.apiUrl}/poidsValidationWorkflow`)
      .pipe(
        tap((response) => {
          console.log("test======================================");
  ...
  getPoidsValidationUser(): Observable<any> {
    return this._httpClient
      .get<any>(
        `${environment.apiUrl}/PoidsValidationWorkflows/user/WORKFLOW_SALAIRE`
      )
      .pipe(
        tap((response) => {
          console.log("test======================================");
  ...
  getById(id: string): Observable<any> {
    return this._httpClient
      .get<any>(`${environment.apiUrl}/poidsValidationWorkflow/${id}`)
      .pipe(
        tap((response) => {
  ...
  delete(id: string): Observable<any> {
    return this._httpClient
      .delete<any>(`${environment.apiUrl}/poidsValidationWorkflow/${id}`)
      .pipe(
        catchError((error) => {
          throw error;
        }),
