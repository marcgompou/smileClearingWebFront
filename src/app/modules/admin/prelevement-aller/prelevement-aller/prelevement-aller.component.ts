...
export class PrelevementAllerComponent implements OnInit, OnDestroy {
  ...
  convertDateToDateTime(dateStr: string): string {
    if (dateStr.length !== 6) {
      ...
      this.alert = {
        type: "error",
        message:
          "La chaîne de date opération doit avoir une longueur de 6 caractères.",
      };
      this.showAlert = true;
  ...
  extractHeaderValues(headerLine: string) {
    ...
    this.headerData = {
      ...
      dateEmission: this.convertDateToDateTime(
        headerLine.substring(8, 14).trim()
      ),
      ...
      dateOper: this.convertDateToDateTime(
        headerLine.substring(dateOperStartIndex, dateOperStartIndex + 6).trim()
      ),
      zoneVide: "zoneVide",
    };
  }
  ...
  extractTotalData(data: string) {
    ...
    const dateEmission =
      this.convertDateToDateTime(data.substring(8, 14).trim()) || null;
