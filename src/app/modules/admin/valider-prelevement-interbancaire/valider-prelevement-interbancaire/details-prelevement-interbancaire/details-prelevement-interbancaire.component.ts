...
@Component({
  selector: 'app-details-prelevement-interbancaire',
  templateUrl: './details-prelevement-interbancaire.component.html',
  styleUrls: ['./details-prelevement-interbancaire.component.scss']
})
export class DetailsPrelevementInterbancaireComponent implements OnInit {
  @ViewChild('matDrawer', { static: true }) matDrawer: MatDrawer;
  drawerMode: 'side' | 'over';
  prelevementInterbancaireData: any;
  ...
  ngOnInit(): void {
    ...
    this._validerPrelevementInterbancaireService.prelevementRemise$.pipe(takeUntil(this._unsubscribeAll)).subscribe((response)=>{
      console.log("------------------------------------------------------------->",response?.data) 
      this.prelevementInterbancaireData=response?.data;
  ...
  validerPrelevement(){
    console.log("valider prelevement id", this.prelevementInterbancaireData);
    this._validerPrelevementInterbancaireService.validerPrelevement(this.prelevementInterbancaireData.id).pipe().subscribe({
      next:(response)=>{
  ...
  telechargerPrelevement(): void {
    ...
    this._validerPrelevementInterbancaireService.telechargerRetourPrelevement(this.prelevementInterbancaireData.id).pipe().subscribe(blob => {
      // Create a temporary anchor element and trigger the download
      const link = document.createElement('a');
      link.href = window.URL.createObjectURL(blob);
      link.download = this.prelevementInterbancaireData.nomfichier+".emi"; // Set the desired file name
      console.log("statut prel",this.prelevementInterbancaireData.statut);
      link.click();
    });
  }
  telechargerRelance(): void {
    this._validerPrelevementInterbancaireService.telechargerRelance(this.prelevementInterbancaireData.id).pipe().subscribe();
