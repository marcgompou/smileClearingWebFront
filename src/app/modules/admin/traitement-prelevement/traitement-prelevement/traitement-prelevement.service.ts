...
export class TraitementPrelevementService {
  private _prelevements: BehaviorSubject<Prelevement[] | null> =
    new BehaviorSubject(null);
  private _titulaire: BehaviorSubject<any> = new BehaviorSubject(null);
  private _prelevementAvalides: BehaviorSubject<Prelevement[] | null> =
    new BehaviorSubject(null);
  private _prelevement: BehaviorSubject<Prelevement | null> =
    new BehaviorSubject(null);
  ...
  getPrelevementAtraitement(): Observable<any> {
    return this._httpClient
      .get<any>(`${environment.apiUrl}/prelevement/prelevement?statut=3`)
      .pipe(
        tap((response) => {
          console.log("test======================================");
  ...
  cloturerPrelevement(idprelevement: string) {
    return this._httpClient
      .put<any>(
        `${environment.apiUrl}/prelevement/admin/cloture/${idprelevement}`,
        null
      )
      .pipe(
        tap((response) => {
  ...
  telechargerPrelevementValider(id: string): Observable<Blob> {
    ...
    return this._httpClient.get(
      `${environment.apiUrl}/prelevement/admin/telechargement/${id}`,
      { responseType: "blob" }
    );
  }
  telechargerRelance(id: string): Observable<void> {
    return this._httpClient
      .get(
        `${environment.apiUrl}/prelevement/admin/telechargementReprise/${id}`,
  ...
  getPrelevementATraiter(id): Observable<any> {
    return this._httpClient
      .get<any>(`${environment.apiUrl}/prelevement/admin/atraiter/${id}`)
      .pipe(
        tap((response) => {
          console.log("test======================================");
          console.log(response);
