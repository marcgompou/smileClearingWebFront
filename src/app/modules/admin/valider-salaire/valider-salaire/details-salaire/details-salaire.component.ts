...
export class DetailsSalaireComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._validerSalaireService.salaireRemise$
    ...
    this._tableDataService.datas$
      ...
      .subscribe((response) => {
        ...
        if (result?.data?.length > 0) {
          findPrelevToRelance = result?.data?.findIndex(
  ...
  AnnulerSalaire() {
    console.log("valider salaire id", this.salaireData);
    this._validerSalaireService
      .validerSalaire(this.salaireData.id)
      .pipe()
      .subscribe({
        next: (response) => {
          console.log(response);
  ...
  getHistoriqueSalaire() {
    this._validerSalaireService.suiviSalaire$
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe((response) => {
        ...
        this.totalStepsSalaire =
          result.data.validators.length + result.data.historiques.length;
        console.log("totalSteps===>", this.totalStepsSalaire);
        this._changeDetectorRef.markForCheck();
      });
  }
  getPoidsValidationUser() {
    this._poidsValidationUser.poidsValidationUser$
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe({
        next: (response: any) => {
  ...
  telechargerSalaire(): void {
    this._validerSalaireService
      .telechargerRetourSalaire(this.salaireData.id)
      .pipe()
      .subscribe((blob) => {
  ...
  telechargerRelance(): void {
    this._validerSalaireService
      .telechargerRelance(this.salaireData.id)
      .pipe()
      .subscribe();
  }
}
