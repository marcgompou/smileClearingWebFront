...
import { RelancePrelevementService } from '../relance-prelevement.service';
import { TableDataService } from '../../common/table-data/table-data.services';



@Component({
  selector: 'app-relance-prelevement',
  templateUrl: './relance-prelevement.component.html',
  styleUrls: ['./relance-prelevement.component.scss'],
...
export class RelancePrelevementComponent  implements OnInit, OnDestroy  {
  ...
  constructor(
    ...
    private _relancePrelevement: RelancePrelevementService,
  ...
  onSubmit() { 
   ...
   this._relancePrelevement.createPrelevement(data).subscribe({
      next: (data) => {
        this.clearFile();
