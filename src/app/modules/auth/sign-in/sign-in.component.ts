...
export class AuthSignInComponent implements OnInit {
      ...
      .subscribe(
        ...
        (response) => {
          ...
          switch (response.status) {
            ...
            case 500:
              this.displayErrors(
                "Erreur interne du serveur, veuillez contacter le service informatique !"
              );
              break;
            default:
              this.displayErrors(
                "Impossible d'établir la connexion,veuillez verifier vos paramètres reseaux"
              );
              break;
          }
        }
  ...
  changePassword(): void {
    ...
    // Send new Password
    this._authService.changePasword(finput).subscribe(
      (response) => {
  ...
  verifyCode(): void {
    ...
    // verify MFA
    this._authService.verifyMFA(this.login2FACodeForm.value["code"]).subscribe(
      (response) => {
